...
void test_relaxed_chamfer_retriever_simple() {
    std::vector<float> a_1 = {1.0, 2.0, 3.0};
    std::vector<float> a_2 = {1.0, -2.0, 3.0};
    std::vector<float> b_1 = {4.0, 5.0, 6.0};
    std::vector<float> b_2 = {4.0, -5.0, 6.0};
    ...
    dataset.push_back(B);
    std::vector<std::string> doc_ids = {"1", "2"};
    RelaxedChamferRetriever relaxedChamferRetriever(3, 500, 1);
    relaxedChamferRetriever.index_dataset(dataset, doc_ids);
    std::vector<std::string> result = relaxedChamferRetriever.get_top_k(A, 1);
    assert(result.size() == 1);
    assert(result[0] == "1");
    std::cout << "✅ test_relaxed_chamfer_retriever_simple passed" << std::endl;
}
